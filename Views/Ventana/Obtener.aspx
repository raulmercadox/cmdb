...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <div class="panel panel-default">
        <div class="panel-heading">
            <h3 class="panel-title">Datos de la Ventana de Ejecución</h3>
        </div>
        <div class="panel-body">
            <form name="frmDatosVentana" id="frmDatosVentana" method="post" action="<%=Url.Content("~/Ventana/Actualizar") %>" class="form-horizontal">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="cboHoraDesde" name="cboHoraDesde"><%
                                                                                                      ...
                                                                                                      {
                                                                                                          int m = i * 5;%>
                                                                                                          <option value="<%=m.ToString().PadLeft(2,'0') %>" <%=minutoDesde==i*5?"selected='selected'":"" %>><%=m.ToString().PadLeft(2,'0') %></option>
                                                                                                      <%} %></select>
                        </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-xs-9 col-sm-9 col-md-8 col-lg-6">
                    <select class="form-control" id="cboHoraHasta" name="cboHoraHasta"><option value='-1' <%=horaHasta==-1?"selected='selected'":"" %>></option><%
                                                                                                      for (int i = 0; i <= 23; i++)
                                                                                                      {%>
                                                                                                          <option value="<%=i.ToString().PadLeft(2,'0') %>" <%=horaHasta==i?"selected='selected'":"" %>><%=i.ToString().PadLeft(2,'0') %></option>
                                                                                                      <%} %></select>:<select class="form-control" id="cboMinutoHasta" name="cboMinutoHasta"><option value='-1' <%=minutoHasta==-1?"selected='selected'":"" %>></option><%
                                                                                                      for (int i = 0; i <= 11; i++)
                                                                                                      {
                                                                                                          int m = i * 5;%>
                                                                                                          <option value="<%=m.ToString().PadLeft(2,'0') %>" <%=minutoHasta==i*5?"selected='selected'":"" %>><%=m.ToString().PadLeft(2,'0') %></option>
                                                                                                      <%} %></select>
                    </div>
                </div>  
            </form>
        ...
        <div class="panel-footer">
            <input type="button" class="btn btn-primary" name="btnActualizarVentana" id="btnActualizarVentana" value="Actualizar" />
        </div>
    </div>
